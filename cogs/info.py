...
class InfoCog(commands.Cog):
    ...
    @app_commands.command(
        name="info",
        description="Ver tu configuración y permisos",
    )
    async def info(self, interaction: discord.Interaction):
        ...
        owner_id, _, sesion = obtener_contexto_usuario(
            interaction.user.id
        )
        ...
        if permisos:
            usuarios_str = "\n".join(
                [f"• <@{uid}>" for uid in permisos[:5]]
            )
            if len(permisos) > 5:
                usuarios_str += f"\n... y {len(permisos) - 5} más"
            embed.add_field(
                name="Lista de Autorizados",
                value=usuarios_str,
                inline=False,
            )
    ...
    async def ayuda(self, interaction: discord.Interaction):
        ...
        embed = discord.Embed(
            ...
            description=(
                "Bot para controlar GitHub Codespaces desde Discord"
            ),
        ...
        embed.add_field(
            ...
            value=(
                "`/setup token:` - Configura tu token de GitHub\n"
        ...
        embed.set_footer(
            text="doce|tools v2 - Control de Codespaces multiusuario"
        )
