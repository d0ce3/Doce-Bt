...
from utils.github_api import iniciar_codespace, detener_codespace, estado_codespace
...
class CodespaceControlCog(commands.Cog):
    ...
    @app_commands.command(name="start", description="Inicia tu Codespace o uno autorizado")
    async def start(self, interaction: discord.Interaction):
        ...
        if success:
            embed = crear_embed_exito(
                ...
                f"⏳ Espera ~30 segundos para que esté completamente listo.",
                footer="Usa /status para verificar el estado"
            )
            await interaction.followup.send(embed=embed)

            await enviar_log_al_propietario(self.bot, codespace, f"Tu Codespace fue iniciado por <@{calling_id}>")
        else:
            embed = crear_embed_error(
                "❌ Error al Iniciar",
                f"**Codespace:** `{codespace}`\n\n**Error:** {mensaje}"
    ...
    @app_commands.command(name="stop", description="Detiene tu Codespace o uno autorizado")
    async def stop(self, interaction: discord.Interaction):
        ...
        if success:
            ...
            await enviar_log_al_propietario(self.bot, codespace, f"Tu Codespace fue detenido por <@{calling_id}>")
        else:
            embed = crear_embed_error(
                "❌ Error al Detener",
                f"**Codespace:** `{codespace}`\n\n**Error:** {mensaje}"
    ...
    @app_commands.command(name="status", description="Consulta el estado de tu Codespace o uno autorizado")
    async def status(self, interaction: discord.Interaction):
        ...
        embed.add_field(name="Sesión", value=tiempo_restante, inline=True)
        if error:
            embed.add_field(name="⚠️ Advertencia", value=error, inline=False)
        await interaction.followup.send(embed=embed)
