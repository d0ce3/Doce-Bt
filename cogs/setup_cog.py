...
class SetupCog(commands.Cog):
    ...
    async def setup(self, interaction: discord.Interaction, token: str):
        ...
        sesiones[user_id] = {
            ...
            "expira_vinculacion": sesiones.get(user_id, {}).get(
                "expira_vinculacion",
    ...
    @app_commands.describe(
        codespace="Nombre de tu Codespace (déjalo vacío para ver lista)"
    )
    async def vincular(
        self,
        ...
        if not codespace:
            ...
            lista = "\n".join(
                [
                    f"• `{c['name']}` Estado: **{c['state']}**"
                    for c in codespaces[:10]
                ]
            )
        ...
        permisos_previos = vinculaciones.get(user_id, {}).get(
            "permisos", []
        )
    ...
    @app_commands.command(
        name="refrescar",
        description="Extiende 5 horas la validez de tu token de GitHub",
    )
    async def refrescar(self, interaction: discord.Interaction):
        ...
        sesiones[user_id]["expira_token"] = (
            datetime.now() + timedelta(hours=5)
        ).isoformat()
        safe_save(SESIONES_FILE, sesiones)

        await interaction.response.send_message(
            "✅ Tu token ha sido renovado por 5 horas.",
            ephemeral=True,
        )
